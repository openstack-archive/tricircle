...
class DistributedRouterNotHostedByL3Agent(exceptions.Conflict):
    message = _("The Distributed router %(router_id)s can't be hosted "
                "on Centralised L3 agent %(agent_id)s.")


class RouterNotHostedByDistributedL3Agent(exceptions.Conflict):
    message = _("The Non-Distributed router %(router_id)s can't be hosted "
                "on Distributed L3 agent %(agent_id)s.")
...
class NoSnatEnabledL3Agent(exceptions.Conflict):
    message = _("No snat enabled l3-agents for the router %(router_id)s.")
