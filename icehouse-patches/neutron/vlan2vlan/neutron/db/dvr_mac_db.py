...
class DVRDbMixin(ext_dvr.DVRMacAddressPluginBase):
    ...
    def get_dvr_mac_address_by_next_hop(self, context, next_hop):
        if not next_hop:
            LOG.warn(_("get_dvr_mac_address_by_host, host input is empty"))
            return

        try:
            return self._get_dvr_mac_address_by_host(context, next_hop)
        except ext_dvr.DVRMacAddressNotFound:
            return self._create_dvr_mac_for_extern_ip(context, next_hop)
    ...
    def get_compute_ports_on_host_by_subnet(self, context, host, subnet):
        # FIXME(vivek): need to optimize this code to do away two-step
        # filtering
        vm_ports_by_host = []
