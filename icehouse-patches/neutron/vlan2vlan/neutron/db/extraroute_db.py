...
from neutron.db import dvr_mac_db as dvr_db
    ...
    def _validate_routes(self, context,
        ...
        for route in routes:
            if(cfg.CONF.cascade_str == 'cascaded'
               ...
               and netaddr.all_matching_cidrs(route['nexthop'],
                                              [extern_relay_cidr])):
                continue
            self._validate_routes_nexthop(
                cidrs, ips, routes, route['nexthop'])

    def get_dvr_mac_for_remote_extern_ip(self, context, remote_cidr, nexthop):
        return(self.get_dvr_mac_address_by_next_hop(context, nexthop))
    ...
    def _update_extra_routes(self, context, router, routes):
        ...
        for route in added:
            if(cfg.CONF.cascade_str == 'cascaded'
               ...
               and netaddr.all_matching_cidrs(route['nexthop'],
                                              [extern_relay_cidr])):
                dvr_mac = self.get_dvr_mac_for_remote_extern_ip(
                    context,
                    route['destination'],
                    route['nexthop'])
                if dvr_mac:
                    self.extern_ip_dvr_mac_map[route['nexthop']] = \
                        dvr_mac.get('mac_address')
        ...
        if(cfg.CONF.cascade_str == 'cascaded' and extern_relay_cidr):
            ...
            for route in extra_routes:
                if(netaddr.all_matching_cidrs(route['nexthop'],
                                              [extern_relay_cidr])):
                    dvr_mac = ExtraRoute_db_mixin.extern_ip_dvr_mac_map.get(
                        route['nexthop'])
                    routes_list.append({'destination': route['destination'],
                                        'nexthop': route['nexthop'],
                                        'nexthop_dvr_mac': dvr_mac,
                                        'onlink': True})
                else:
                    routes_list.append({'destination': route['destination'],
                                        'nexthop': route['nexthop']})
