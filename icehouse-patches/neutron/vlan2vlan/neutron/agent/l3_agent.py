...
class NextHopMACPriority(object):

    def __init__(self, nexthop, dvr_mac, priority):
        self.nexthop = nexthop
        self.dvr_mac = dvr_mac
...
class RouterInfo(object):
    ...
    def __init__(self, router_id, root_helper, use_namespaces, router):
        ...
        self.next_hop_ip_mac_map = {}
        self.extern_ip_interface_name = None
        self.available_ip_rule_priority = set(moves.xrange(32, 35535))
    ...
    OPTS = [
        ...
        cfg.StrOpt('metadata_proxy_socket',
                   ...
                   help=_('Location of Metadata Proxy UNIX domain '
                          'socket')),
        # added by j00209498  ---begin
        cfg.StrOpt('notify_l2_file_path',
                   default='/var/',
                   help=_('Location of  notify_l2_file_path')),
        cfg.StrOpt('notify_l2_file_name',
                   default='notify_l2_agent_info.json',
                   help=_('File name of  notify_l2_file')),
        # added by j00209498  ---end
    ]
...
#                                   internal_cidrs, interface_name)
    ...
    def _external_gateway_added(self, ri, ex_gw_port, interface_name,
        ...
        if not ip_lib.device_exists(interface_name,
            ...
            self.driver.plug(ex_gw_port['network_id'],
                             ...
                             namespace=ns_name,
                             prefix=EXTERNAL_DEV_PREFIX)
            ri.extern_ip_interface_name = interface_name
    ...
    def _update_static_arp_entry(self, ri, operation, ip, mac):
        """Add or delete arp entry into router namespace."""
        if ri.extern_ip_interface_name:
            ip_cidr = str(ip) + '/32'
            try:
                # TODO(mrsmith): optimize the calls below for bulk calls
                net = netaddr.IPNetwork(ip_cidr)
                interface_name = ri.extern_ip_interface_name
                device = ip_lib.IPDevice(interface_name, self.root_helper,
                                         namespace=ri.ns_name)
                if operation == 'add':
                    device.neigh.add(net.version, ip, mac)
    ...
    def _update_ip_rule(self, ri, operation, dest_ip, priority):
        ipRule = ip_lib.IpRule(self.root_helper, namespace=ri.ns_name)
        table = priority
        rule_pr = priority
        if(operation == 'add'):
            ipRule.add_rule_to(dest_ip, table, rule_pr)
        else:
            ipRule.delete_rule_priority(rule_pr)

    def _update_route_entry(self, ri, operation, gateway_ip, onlink, table):
        device = ip_lib.IPDevice(ri.extern_ip_interface_name,
                                 self.root_helper,
                                 namespace=ri.ns_name)
        if(operation == 'add'):
            device.route.add_gateway_onlink(gateway_ip, 'onlink', table)
        if(operation == 'delete'):
            device.route.del_route_table(table)

    def _update_ip_neigh_and_ip_rule(self, ri, opration, route):
        next_hop = route['nexthop']
        dest_cidr = route['destination']
        dvr_mac = route['nexthop_dvr_mac']
        nhp = ri.next_hop_ip_mac_map.get(next_hop)
        if(opration == 'add'):
            if(nhp):
                self._update_ip_rule(ri, 'add', dest_cidr, nhp.priority)
            else:
                priority = ri.available_ip_rule_priority.pop()
                nhp = NextHopMACPriority(next_hop, dvr_mac, priority)
                ri.next_hop_ip_mac_map[next_hop] = nhp
                self._update_static_arp_entry(ri, 'add', next_hop, dvr_mac)
                self._update_route_entry(ri, 'add', next_hop,
                                         'onlink', priority)
                self._update_ip_rule(ri, 'add', dest_cidr, priority)
        if(opration == 'delete'):
            self._update_route_entry(ri, 'delete', next_hop, '', nhp.priority)
            self._update_static_arp_entry(ri, 'delete', next_hop, dvr_mac)
            if(nhp):
                self._update_ip_rule(ri, 'delete', dest_cidr, nhp.priority)
                ri.next_hop_ip_mac_map.pop(next_hop, None)

    def write_to_file(self, path, contents):
        with open(path, 'w') as f:
            f.write(contents)

    def _notify_l2_agent(self, ri):
        if not ri:
            return
        ext_inter_name = ri.extern_ip_interface_name
        if(not ext_inter_name):
            return
        if(len(ri.internal_ports) <= 0):
            #self.notify_l2_agent_info.pop(ext_inter_name, None)
            self.notify_l2_agent_info[ext_inter_name] = {}
            ri.next_hop_ip_mac_map = {}
        else:
            #        ip_mac_map = self.notify_l2_agent_info.get(ext_inter_name, None)
            #        if(not ip_mac_map):
            self.notify_l2_agent_info[ext_inter_name] = {}
            for ip, nhp in ri.next_hop_ip_mac_map.items():
                self.notify_l2_agent_info[ext_inter_name][nhp.dvr_mac] = ip
        path = os.path.join(self.conf.notify_l2_file_path,
                            self.conf.notify_l2_file_name)
        self.write_to_file(path, jsonutils.dumps(self.notify_l2_agent_info))
    ...
    def routes_updated(self, ri):
        ...
        for route in adds:
            ...
            if(route.get('nexthop_dvr_mac') and route.get('onlink')):
                self._update_ip_neigh_and_ip_rule(ri, 'add', route)
            else:
        ...
        for route in removes:
            ...
            if(route.get('nexthop_dvr_mac') and route.get('onlink')):
                self._update_ip_neigh_and_ip_rule(ri, 'delete', route)
            else:
                self._update_routing_table(ri, 'delete', route)
        ri.routes = new_routes
        self._notify_l2_agent(ri)
