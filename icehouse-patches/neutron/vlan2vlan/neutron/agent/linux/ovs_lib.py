...
class OVSBridge(BaseOVS):
    ...
    def add_flowkey_tunnel_port(self, port_name,
                                tunnel_type=p_const.TYPE_GRE,
                                vxlan_udp_port=constants.VXLAN_UDP_PORT,
                                dont_fragment=True):
        vsctl_command = ["--", "--may-exist", "add-port", self.br_name,
        ...
        vsctl_command.append(("options:df_default=%s" %
                              bool(dont_fragment)).lower())
        vsctl_command.extend(["options:remote_ip=flow",
                              "options:key=flow"])
        self.run_vsctl(vsctl_command)
        ofport = self.get_port_ofport(port_name)
        if (tunnel_type == p_const.TYPE_VXLAN and
...
def exec_host_ip_config_up(eth_port, num, gw_ip):
    port = eth_port + ':' + str(num)
    args = ["ifconfig", port, '%s/24' % gw_ip]
    try:
        return utils.execute(args).strip()
    except Exception:
        LOG.exception(_("Interface %s not found."), eth_port)
        return None


def exec_host_ip_config_down(eth_port, num):
    port = eth_port + ':' + str(num)
    args = ["ifconfig", port, 'down']
    try:
        return utils.execute(args).strip()
    except Exception:
        LOG.exception(_("Interface %s not found."), eth_port)
