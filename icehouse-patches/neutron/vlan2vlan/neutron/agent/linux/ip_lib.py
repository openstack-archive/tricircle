...
class IpRule(IPWrapper):
    ...
    def add_rule_to(self, ip, table, rule_pr):
        args = ['add', 'to', ip, 'lookup', table, 'priority', rule_pr]
        ip = self._as_root('', 'rule', tuple(args))
        return ip
...
class IpRouteCommand(IpDeviceCommandBase):
    ...
    def add_gateway_onlink(self, gateway, onlink='onlink', table=None):
        args = ['replace', 'default', 'via', gateway]
        args += ['dev', self.name]
        if onlink:
            args += ['onlink']
    ...
    def del_route_table(self, table):
        args = ['del', 'table', table]
        self._as_root(*args)
