...
from neutron.db import cascade_db
...
class Ml2Plugin(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def _process_port_binding(self, mech_context, context, attrs):
        ...
        profile_set = profile is not None and \
            profile is not attributes.ATTR_NOT_SPECIFIED
    ...
    def _process_dvr_port_binding(self, mech_context, context, attrs):
        ...
        if binding.vif_type != portbindings.VIF_TYPE_UNBOUND:
            if (not host_set and binding.segment and
                    ...
                    validate_port_binding(mech_context)):
                return False
    ...
    def _check_port_binding_az_valid(self, context, network, binding_host):
        net_type = network['provider:network_type']
        seg_id = network['provider:segmentation_id']
        net_id = network['id']
        if (net_type == 'vxlan' and self.is_big2layer_vni(seg_id)):
            return True
        elif(net_type in ['vlan', 'vxlan']):
            host = self.get_binding_az_by_network_id(context, net_id)
            if(not host):
                self.add_binding_az_network_id(context, binding_host, net_id)
            elif(host and host != binding_host):
                return False
            return True
        #net_type == 'local' or net_type == 'flat' or 'gre'
        return True
    ...
    def create_port(self, context, port):
        ...
        with session.begin(subtransactions=True):
            ...
            if('compute' in attrs['device_owner']):
                binding_host = attrs.get('binding:host_id', None)
                if cfg.CONF.cascade_str == 'cascading' and binding_host:
                    is_continue = True
                    is_continue = self._check_port_binding_az_valid(
                        context,
                        network,
                        binding_host)
                    if(not is_continue):
                        raise exc.PortBindAZError(
            ...
            self._process_port_create_extra_dhcp_opts(context, result,
                                                      dhcp_opts)
            #self.l3_agent_notify_for_vmarp_table(context, result['id'], 'add')
            if mech_context:
    ...
    def port_bound_to_host(self, context, port_id, host):
        try:
            ...
            if port['device_owner'] == const.DEVICE_OWNER_DVR_INTERFACE:
                ...
                for b in bindings:
                    if (b.host == host):
                        # LOG.debug(_("Binding with host %s exists for port %s")
                        #          % (host, port_id))
                        return True
                LOG.debug(_("No Binding exists for port %s"), port_id)
                return False
            else:
                port_host = db.get_port_binding_host(port_id)
                return (port_host == host)
        except exc.PortNotFound:
            LOG.debug(_("Port not found %s"), port_id)
            return False
