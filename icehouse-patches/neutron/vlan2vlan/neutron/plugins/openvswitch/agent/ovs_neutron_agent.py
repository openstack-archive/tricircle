...
class GatewayPortExternInfo:

    def __init__(self, port, ip, vlan_id,
                 num=None, extern_ip_mac_mapping=None):
        self.port = port
        self.gateway_ip = ip
        self.local_vlan = vlan_id
        self.eth_num = num
        if(not extern_ip_mac_mapping):
            self.extern_ip_mac_mapping = {}
        else:
            self.extern_ip_mac_mapping = extern_ip_mac_mapping
...
class OVSNeutronAgent(n_rpc.RpcCallback,
    ...
    def __init__(self, integ_br, tun_br, local_ip,
        ...
        self.tun_br_flowkey_ofport = None
        self.gateway_port_extern_info = {}
        self.extern_eth = cfg.CONF.AGENT.extern_net_port
        self.available_eth_num_list = set(moves.xrange(0, 1000))
    ...
    def extern_ip_host_setting_up(self, extern_ip):
        if not self.available_eth_num_list:
            LOG.error(_("No eth num available for extern_Ip=%s"), extern_ip)
            return None
        num = self.available_eth_num_list.pop()
        ovs_lib.exec_host_ip_config_up(self.extern_eth, num, extern_ip)
        return num

    def extern_ip_host_setting_down(self, eth_num, extern_ip):
        self.available_eth_num_list.add(eth_num)
        ovs_lib.exec_host_ip_config_down(self.extern_eth, eth_num)

    def bind_extern_tun_port_process(self, port, fixed_ips,
                                     device_owner, local_vlan_id):
        if(device_owner == q_const.DEVICE_OWNER_ROUTER_GW):
            # gateway port must have only one fixed IP,
            # use fixed_ips[0]
            gw_port_ip = fixed_ips[0]['ip_address']
            eth_num = self.extern_ip_host_setting_up(gw_port_ip)
            self.gateway_port_extern_info[port.port_name] = \
                GatewayPortExternInfo(port, gw_port_ip, local_vlan_id, eth_num)
            self.tun_br.add_flow(table=constants.DVR_PROCESS,
                                 priority=2,
                                 dl_vlan=local_vlan_id,
                                 dl_dst='fa:16:3f:00:00:00/ff:ff:ff:00:00:00',
                                 actions="resubmit(,%s)" %
                                 (constants.FLOW_KEY_TABLE))

            network_type = p_const.TYPE_VXLAN
            segmentation_id = cfg.CONF.AGENT.extern_tun_vni
            self.tun_br.add_flow(
                table=constants.TUN_TABLE[network_type],
                priority=1,
                tun_id=segmentation_id,
                ...
                actions="mod_vlan_vid:%s,"
                "resubmit(,%s)" %
                        (local_vlan_id, constants.DVR_NOT_LEARN))

            self.int_br.add_flow(table=constants.DVR_TO_DST_MAC,
                                 priority=2,
                                 dl_vlan=local_vlan_id,
                                 actions="mod_dl_dst:%s,"
                                 "NORMAL" % port.vif_mac)

    def unbind_extern_tun_port_process(self, port, local_vlan_id):
        gpe = self.gateway_port_extern_info.pop(port.port_name, None)
        if(gpe):
            # gateway port must have only one fixed IP,
            # use fixed_ips[0]
            gw_port_ip = gpe.gateway_ip
            self.extern_ip_host_setting_down(gpe.eth_num, gw_port_ip)
            self.tun_br.delete_flows(
            ...
            self.int_br.delete_flows(table=constants.DVR_TO_DST_MAC,
                                     dl_vlan=local_vlan_id)
    ...
    def port_bound(self, port, net_uuid,
        ...
        self.bind_extern_tun_port_process(port, fixed_ips,
                                          device_owner,
    ...
    def port_unbound(self, vif_id, net_uuid=None):
        ...
        if not self.local_vlan_map.get(net_uuid):
            LOG.info(_('port_unbound(): net_uuid %s not'
                       ' in local_vlan_map'), net_uuid)
            return
        ...
        if vif_id in lvm.vif_ports:
            ...
            self.unbind_extern_tun_port_process(vif_port, lvm.vlan)
    ...
    def setup_tunnel_br(self, tun_br=None):
        ...
        self.setup_flowkey_tun_port('flowkey_tun_port', p_const.TYPE_VXLAN)
    ...
    def setup_flowkey_tun_port(self, port_name, tunnel_type):
        ofport = self.tun_br.add_flowkey_tunnel_port(port_name,
                                                     tunnel_type,
                                                     self.vxlan_udp_port,
                                                     self.dont_fragment)
        ofport_int = -1
        ...
        if ofport_int < 0:
            LOG.error(_("Failed to set-up %(type)s flow_key tunnel port"),
                      {'type': tunnel_type})
            return 0

        self.tun_br_flowkey_ofport = ofport
    ...
    def _parse_json_file(self, json_file):
        try:
            ...
            LOG.error(_('parse json file failed! json_file= %s'), json_file)
            return None

        return json_dict

    def scan_notify_l2_agent_file(self):
        fp = os.path.join(cfg.CONF.AGENT.notify_l2_file_path,
                          cfg.CONF.AGENT.notify_l2_file_name)
        return self._parse_json_file(fp)

    def process_extern_port_info(self):
        extern_port_info = self.scan_notify_l2_agent_file()
        if not extern_port_info:
            return

        for port_name, mac_ip_map in extern_port_info.items():
            gpe = self.gateway_port_extern_info.get(port_name, None)
            if not gpe:
                continue
            add_ports = set(mac_ip_map.keys()) - \
                set(gpe.extern_ip_mac_mapping.keys())
            remove_ports = set(gpe.extern_ip_mac_mapping.keys()) - \
                set(mac_ip_map.keys())
            for mac in list(add_ports):
                dst_extern_ip = mac_ip_map.get(mac)
                segmentation_id = cfg.CONF.AGENT.extern_tun_vni
                ofport = self.tun_br_flowkey_ofport
                self.tun_br.add_flow(
                    table=constants.FLOW_KEY_TABLE,
                    priority=1,
                    dl_vlan=gpe.local_vlan,
                    dl_dst=mac,
                    actions="strip_vlan,set_field:%s->tun_id,"
                    "set_field:%s->tun_dst,set_field:%s->tun_src,"
                    "output:%s" %
                    (segmentation_id,
                     dst_extern_ip,
                     gpe.gateway_ip,
                     ofport))
                gpe.extern_ip_mac_mapping[mac] = dst_extern_ip
            for mac in list(remove_ports):
                gpe.extern_ip_mac_mapping.pop(mac, None)
                segmentation_id = cfg.CONF.AGENT.extern_tun_vni
                ofport = self.tun_br_flowkey_ofport
                self.tun_br.delete_flows(table=constants.FLOW_KEY_TABLE,
                                         dl_vlan=gpe.local_vlan,
                                         dl_dst=mac)
