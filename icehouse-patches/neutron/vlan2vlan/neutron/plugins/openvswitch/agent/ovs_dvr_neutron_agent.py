...
class OVSDVRNeutronAgent(dvr_rpc.DVRAgentRpcApiMixin):
    ...
    def setup_dvr_flows_on_integ_tun_br(self):
        ...
        try:
            ...
            details = self.plugin_rpc.\
                get_dvr_mac_address_by_host(self.context, self.host)
        ...
        # Insert 'drop' action as the default for Table 2
        ...
        # Insert 'normal' action as the default for Table 1
...
# add by j00209498
        self.int_br.add_flow(table=constants.LOCAL_SWITCHING,
                             ...
                             dl_src='fa:16:3f:00:00:00/ff:ff:ff:00:00:00',
                             actions="resubmit(,%s)" %
...
#            self.registered_dvr_macs.add(newmac)
    ...
    def is_dvr_router_interface(self, device_owner):
        return (device_owner == q_const.DEVICE_OWNER_ROUTER_INTF_DISTRIBUTED)
    ...
    def _bind_distributed_router_interface_port(self, port, fixed_ips,
        ...
        local_compute_ports = self.plugin_rpc.\
            get_compute_ports_on_host_by_subnet(self.context,
                                                self.host,
                                                subnet_uuid)
        ...
        # create rule in Table LOCAL_SWITCHING to forward
        # broadcast/multicast frames from dvr router interface to
        # appropriate local tenant ports
    ...
    def bind_port_to_dvr(self, port, network_type, fixed_ips,
        ...
        if device_owner == q_const.DEVICE_OWNER_ROUTER_INTF_DISTRIBUTED:
            self._bind_distributed_router_interface_port(port, fixed_ips,
                                                         device_owner,
                                                         local_vlan_id)

        if 'compute' in device_owner:
    ...
    def unbind_port_from_dvr(self, vif_port, local_vlan_id):
        ...
        if not vif_port:
            LOG.debug("DVR: VIF Port not available for delete %s", vif_port)
            return

        # Handle  port removed use-case
        if vif_port.vif_id not in self.local_ports:
            LOG.debug("DVR: Non distributed port, ignoring %s", vif_port)
            return

        ovsport = self.local_ports[vif_port.vif_id]

        if ovsport.get_device_owner() == \
                q_const.DEVICE_OWNER_ROUTER_INTF_DISTRIBUTED:
            self._unbind_distributed_router_interface_port(vif_port,
                                                           local_vlan_id)

        if 'compute' in ovsport.get_device_owner():
            self._unbind_compute_port_on_dvr_subnet(vif_port,
                                                    local_vlan_id)

        if ovsport.get_device_owner() == q_const.DEVICE_OWNER_ROUTER_SNAT:
