...
_CODE_DIR="../neutron/"
_BACKUP_DIR="${_NEUTRON_INSTALL}/.neutron-cascaded-openvswitch-agent-installation-backup"

#_SCRIPT_NAME="${0##*/}"
#_SCRIPT_LOGFILE="/var/log/neutron-cascaded-openvswitch-agent/installation/${_SCRIPT_NAME}.log"
...
if [ -d "${_BACKUP_DIR}/neutron" ] ; then
    echo "It seems neutron-cascaded-openvswitch-agent has already been installed!"
...
echo "restarting cascaded neutron openvswitch agent..."
service openstack-neutron-openvswitch-agent restart
if [ $? -ne 0 ] ; then
    echo "There was an error in restarting the service, please restart cascade neutron openvswitch-agent manually."
    exit 1
fi
