...
class OVSNeutronAgent(n_rpc.RpcCallback,
    ...
    def port_bound(self, port, net_uuid,
        ...
        # may be flush subnet_map info --add by j00209498
#        for subnet_ip in cascaded_port_info['fixed_ips']:
#            subnet_id = subnet_ip('subnet_id')
#            if(not lvm.get(subnet_id)):
#                subnet_info = self.get_cascaded_subnet_info(subnet_id)
#                if len(subnet_info.get('subnets')) > 0:
#                    lvm.cascaded_subnet[subnet_id] = subnet_info['subnets']['cidr']
...
#                                                local_vlan_id=lvm.vlan)
    ...
    def get_port_id_from_profile(self, profile):
        ...
        return profile.get('cascading_port_id')
