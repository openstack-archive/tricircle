...
_CONF_BACKUP_DIR="`dirname ${_NEUTRON_CONF_DIR}`/.neutron-cascading-l2proxy-installation-backup"
_CODE_DIR="../neutron/"
_BACKUP_DIR="${_NEUTRON_INSTALL}/.neutron-cascading-l2proxy-installation-backup"
#_SCRIPT_NAME="${0##*/}"
...
if [ -d "${_BACKUP_DIR}/neutron" -o -d "${_CONF_BACKUP_DIR}/neutron" ] ; then
    echo "It seems l2proxy has already been installed!"
...
echo "stoping neutron-openvswitch-agent"
service neutron-plugin-openvswitch-agent stop
ps -efw|grep l2_proxy.py|grep -v grep|awk '{print $2}'|xargs kill -9

echo "starting neutron l2-proxy..."
nohup  /usr/bin/python ${_NEUTRON_DIR}/plugins/l2_proxy/agent/l2_proxy.py --config-file ${_NEUTRON_CONF} --config-file ${_NEUTRON_L2_PROXY_CONF} >/dev/null 2>&1 &
if [ $? -ne 0 ] ; then
    echo "There was an error in starting the l2-proxy, please start neutron l2-proxy manually."
    exit 1
fi
