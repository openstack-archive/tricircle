...
class GreTunnel(object):
    ...
    def __init__(self, nexthop, gre_device_name):
        self.remote_ip = nexthop
        self.gre_device_name = gre_device_name
        self.extra_route_set = set()
    
    def add_extra_route(self, destination):
        self.extra_route_set.add(destination)
        
    def remove_extra_route(self, destination):
        if(destination in self.extra_route_set):
            self.extra_route_set.remove(destination)
...
class RouterInfo(l3_ha_agent.RouterMixin):
    ...
    def __init__(self, router_id, root_helper, use_namespaces, router,
        ...
        self.next_hop_gre_tunnel = {}
        self.route_cidrs = set()
...
#commented by jiahaojie---begin
            ri.perform_snat_action(self._handle_router_snat_rules,
                                   internal_cidrs, interface_name)
#commented by jiahaojie---end
    ...
    def get_ip_in_hex(self, ip_address):
        try:
            ...
            LOG.warn(_("Unable to create gre tunnel. Invalid remote IP: %s"),
                     ip_address)
            return

    def _update_ip_route(self, ri, operation, dest_cidr, gre_interface):
        snat_ns_name = self.get_snat_ns_name(ri.router['id'])
        device = ip_lib.IPDevice(gre_interface,
                                 self.root_helper,
                                 namespace=snat_ns_name)
        if(operation == 'add'):
            device.route.add_onlink_route(dest_cidr)
        if(operation == 'delete'):
            device.route.delete_onlink_route(dest_cidr)
    
    def _create_gre_tunnel(self, ri, gre_device, remote_ip, local_ip):
        snat_ns_name = self.get_snat_ns_name(ri.router['id'])
        device = ip_lib.IPDevice(gre_device,
                                 self.root_helper,
                                 namespace=snat_ns_name)
        device.tunnel.add('gre', remote_ip, local_ip)
        device.link.set_up()
    
    def _delete_gre_tunnel(self, ri, gre_device):
        snat_ns_name = self.get_snat_ns_name(ri.router['id'])
        if ip_lib.device_exists(gre_device,
                                root_helper=self.root_helper,
                                namespace=snat_ns_name):
            device = ip_lib.IPDevice(gre_device,
                                 self.root_helper,
                                 namespace=snat_ns_name)
            device.tunnel.delete()
   
    def process_route_by_gre_tunnel(self, ri, operation, route):
        next_hop = route['nexthop']
        dest_cidr = route['destination']
        ngt = ri.next_hop_gre_tunnel.get(next_hop)
        if(operation == 'add'):
            if(ngt and (dest_cidr not in ngt.extra_route_set)):
                gre_interface = ngt.gre_device_name
                self._update_ip_route(ri, 'add', dest_cidr, gre_interface)
                ngt.add_extra_route(dest_cidr)
            else:
                gre_interface = 'gre_' + self.get_ip_in_hex(next_hop)
                ngt = GreTunnel(next_hop, gre_interface)
                ri.next_hop_gre_tunnel[next_hop] = ngt
                if not ri.router['gw_port']:
                    return
                self._create_gre_tunnel(ri, gre_interface, next_hop,
                    ri.router['gw_port']['fixed_ips'][0].get('ip_address'))
                self._update_ip_route(ri, 'add', dest_cidr, gre_interface)
                ngt.add_extra_route(dest_cidr)
        if(operation == 'delete'):
            if(ngt):
                gre_interface = ngt.gre_device_name
                self._update_ip_route(ri, 'delete', dest_cidr, gre_interface)
                ngt.remove_extra_route(dest_cidr)
                if(len(ngt.extra_route_set) == 0):
                    self._delete_gre_tunnel(ri, gre_interface)
                    ri.next_hop_gre_tunnel.pop(next_hop)
    ...
    def routes_updated(self, ri):
        ...
        for route in adds:
            ...
            if(route.get('onlink')):
                if(self.conf.agent_mode == 'dvr_snat'):
                    self.process_route_by_gre_tunnel(ri, 'add', route)
                else:
                    continue
            else:
        ...
        for route in removes:
            ...
            if(route.get('onlink')):
                if(self.conf.agent_mode == 'dvr_snat'):
                    self.process_route_by_gre_tunnel(ri, 'delete', route)
                else:
                    continue
